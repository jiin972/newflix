...
const TvshowRowWrapper = styled.div`
  position: relative;
  height: 200px;
  width: calc(100% - 30px);
  margin-right: 30px;
...
function TvSlider({
  ...
  const [dierction, setDirection] = useState(1);
  // Banner에 사용된 Tvshow를 총 목로겡서 제외하기 위한 로직
  const tvshowList = useMemo(() => {
    return data?.results || null;
  }, [data]);
  const filteredTvshow = useMemo(() => {
    if (bannerIdToExclude === null) {
      return tvshowList;
    }
    return tvshowList?.filter((tvshow) => tvshow.id !== bannerIdToExclude);
  }, [tvshowList, bannerIdToExclude]);
  const totalTvshows = filteredTvshow?.length || 0;
  const maxSliderIndex = Math.floor(totalTvshows / offset) - 1;
  const increaseIndex = () => {
                ...
                {filteredTvshow
                  ...
                  .map((tvshow) => (
                    <Box
                      ...
                      $bgPhoto={
                        filteredTvshow
                          ? makeImagePath(tvshow.backdrop_path, "w500")
                          : DEFAULT_IMAGE_URL
                      }
